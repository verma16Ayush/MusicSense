...
# DONE: generate spectrogram
# DONE: Find peaks
# DONE: generate hashes
...
def fingerprint(channel_samples: np.ndarray,
                ...
                save_specgram : bool = SAVE_SPECGRAM) -> List[Tuple[str, int]]:
    """
    ...
    arr2D : np.ndarray = mlab.specgram(
        channel_samples,
        NFFT=wsize,
        Fs=Fs,
    ...
    arr2D = 10 * np.log10(arr2D, out=np.zeros_like(arr2D), where=(arr2D != 0)) # type: ignore

    if save_specgram:
        plt.imsave(f'./plots/{song_name}-specgram.jpg', arr2D) #type: ignore

    local_maxima = get_2D_peaks(arr2D, song_name=song_name, amp_min=amp_min)

    # return hashes
    return generate_hashes(local_maxima, fan_value=fan_value)

def get_2D_peaks(arr2D: np.ndarray, song_name: str | None = None, plot: bool = SHOW_PLOT, amp_min: int = DEFAULT_AMP_MIN) -> List[Tuple[int, int]]:
    """
    ...
    if plot:
        plt.imsave(f'./plots/{song_name}-peaks.jpg', detected_peaks) #type: ignore
        plt.imshow(detected_peaks) #type: ignore
